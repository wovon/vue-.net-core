<template>
  <div class="app-container">
    ...
    <tree-table :data="selectOptionData" ref="selectOptionTable" :evalFunc="func" @func="getRowSelect"  :evalArgs="args" :expandAll="expandAll" border >
    ...
    <el-dialog :title="dialogMap[dialogStatus]" :visible.sync="dialogFromVisible">
      <el-form ref="selectOptionFrom" :model="temp" label-position="right" label-width="80px" style="width: 400px; margin-left:100px;">
        <el-form-item label="名  称" >
          <el-input v-model="temp.text" style="width:450px;"></el-input>
        </el-form-item>
        <el-form-item label="Value" >
          <el-input-number v-model="temp.value" :min="0" :max="10000" label=""></el-input-number>
        </el-form-item>
        <el-form-item label="Code" >
          <el-input-number v-model="temp.code" :min="0" :max="10000" label=""></el-input-number>
        </el-form-item>
        <el-form-item label="上一级" >
          <!-- <treeselect :options="selectOptionData" :multiple="multiple" v-model="temp.parentId" :disable-branch-nodes="false" :show-count="false" placeholder="" :flat="true" :defaultExpandLevel="Infinity" :maxHeight="200" style="width:450px;" /> -->
          <treeselect
          :multiple="true"
        ...
        <!-- <el-form-item label="设置对应子类的ID" >
          <treeselect :options="selectOptionData" v-model="temp.useChildId" :disable-branch-nodes="false" :show-count="false" placeholder="" :flat="true" :defaultExpandLevel="Infinity" :maxHeight="200" style="width:450px;" />
        </el-form-item> -->
    ...
    <!-- <el-dialog title="设置角色" :visible.sync="dialogSetRoleVisible">
      <el-form ref="selectOptionFrom" :model="temp" label-position="left" label-width="80px" style="width: 400px; margin-left:100px;">
...
/**
  Auth: v
  Created: 2019-04-11 14:54
*/
import treeTable from '@/components/TreeTable'
import treeToArray from '@/views/example/table/treeTable/customEval'
import {selectOptionList,selectOptionAdd,selectOptionDelete,selectOptionEdit} from "@/api/selectOption"
...
export default {
  ...
  methods: {//获取操作按钮
    ...
    getSelectOption(){
      selectOptionList().then(response=>{
        //console.log('selectOptionList', response.data.data)
        this.selectOptionData = response.data.data;
      }).catch((e)=>{
        console.log(e)
      })
    },
    ...
    handleElement(code){
      ...
      else if(code==="edit" || code==="see")
      {
        ...
        else if(this.rowSelect.length>1)
        {
          ...
          this.$refs.selectOption.handleClearSelection()
        }
        else
        {
          ...
          this.dialogFromVisible = true
          this.$nextTick(() => {
          this.$refs["selectOptionFrom"].clearValidate()
        });
        }
      }
      // //设置角色
    ...
    handleAdd(){
      ...
      this.$refs["selectOptionFrom"].validate(valid => {
        selectOptionAdd(tempData).then(response=>{
    ...
    handleDelete(){
      ...
      if(_ids.ids!="")
      {
        selectOptionDelete(_ids).then(response=>{
          if(response.data.code===200)
          {
    ...
    handleEidt(){
      ...
      this.$refs["selectOptionFrom"].validate(valid => {
        selectOptionEdit(tempData).then(response=>{
          if(response.data.code===200)
          {
            this.dialogFromVisible = false
