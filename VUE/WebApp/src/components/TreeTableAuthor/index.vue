  ...
  <el-table :data="formatData" :row-style="showRow" v-bind="$attrs">
    ...
    <el-table-column v-else v-for="(column, index) in columns" :key="column.value" :label="column.text" :width="column.width">
      <template slot-scope="scope">
        ...
        <el-checkbox  v-if ="scope.row[column.act]"   :indeterminate="scope.row.isIndeterminate" v-model="scope.row.checkAll" @change="handleCheckAllChange1(scope.$index, scope.row,column.option)" >
          {{scope.row[column.act]}}
        </el-checkbox>
...
export default {
  ...
  methods: {
    ...
    handleCheckAllChange (index, row, opt) {
      ...
      console.log('row', row.selectchecked)
      console.log('opt', opt)
